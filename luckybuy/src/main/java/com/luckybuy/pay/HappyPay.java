...
public class HappyPay {
	...
	public HappyPay(Activity context) {
		this.context = context;
        mBluePay = com.bluepay.pay.BluePay.getInstance();
        bluePayInit = new BluePayInitImpl(context);
	}

    public HappyPay(Activity context, BluePayInit bluePayInit) {
        this(context);
	...
	public void pay_5_THB(Intent intent){
        ...
        final String cardNum = bundle.getString("cardNum");
        final String snNum = bundle.getString("snNum");
        ...
        mBluePay.payByCashcard(context, userID + "",
                order_number, "PropsName",
                PublisherCode.PUBLISHER_HAPPY, cardNum + snNum, null, callback);
